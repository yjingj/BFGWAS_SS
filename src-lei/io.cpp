...
bool ReadFile_anno (const string &file_anno, const string &file_func_code, map<string, int> &mapFunc2Code, vector<bool> &indicator_snp, vector<SNPINFO> &snpInfo, size_t &n_type, vector<size_t> &mFunc, map<string, int> &mapID2num)
{
    ...
    while (!safeGetline(infile, line).eof()) {
        if (line[0] == '#') {
            ...
            if(mapID2num.count(rs) == 0 && mapID2num.count(key) == 0) {
                SwapKey(key);
...
bool ReadFile_vcf (const string &file_vcf, vector<bool> &indicator_idv, vector<bool> &indicator_snp, gsl_matrix *X, const uint ni_test, const uint ns_test, gsl_matrix *K, const bool calc_K, string &GTfield, vector<double> &SNPmean, vector <size_t> &CompBuffSizeVec, const vector <size_t> &SampleVcfPos, const map<string, size_t> &PhenoID2Pos, const vector<string> &VcfSampleID, bool Compress_Flag)
{
    if (GTfield.empty()) {
        GTfield = "GT"; //defalt load GT Data
    }
...
bool ReadFile_score(const string &file_score, vector<SNPINFO> &snpInfo, map<string, size_t> &mapScoreKey2Pos, map<string, size_t> &mapLDKey2Pos, vector<double> &pval_vec, vector<pair<size_t, double> >  &pos_ChisqTest, vector<double> &U_STAT, vector<double> &SQRT_V_STAT,vector<double> &Z_SCORE, vector<double> &xtx_vec, vector<double> &snp_var_vec, size_t &ns_test, size_t &ns_total, vector<double> &mbeta, vector<double> &mbeta_SE, vector <bool> &indicator_snp, const size_t &ni_test, const double &maf_level, const double &hwe_level, const double &pheno_var, const vector< vector<double> >  &LD_ref, const bool &use_xtx_LD)
{
    ...
    while (!safeGetline(infile_score, line).eof()) 
    {
        ...
        else 
        {
            ...
            if(rs.compare(".") == 0 || rs.empty() )
                { rs = key; }
            cout<<key<<endl;
            //Lei's change,important
            //get score through summary statistics
            if((!z_na))
            {
                u_i=z_i*sqrt(ni_test);
                //cout << "u_i= "<< u_i <<endl;
                u_na=false;
                v_i=sqrt(ni_test);
                //cout << "v_i= "<< v_i <<endl; always = 50.3 for 2504 sample size.
                v_na=false;
            }
            ...
            if( u_na && (!beta_na) ){
                ...
                //cout<<"jump in the forth part\n"<<endl;
            	ns_total ++; continue;

            }

            //get sqrt_v_stat through summary statistics
            if (v_na) {
            ...
            //Q:what if z-socre = NA? set as the U_STAT_i/Sqrt(ni_test)?
            //if(z_na){
                //z_i = u_i * (1/sqrt(ni_test));
                //z_na = false;
            //}
    ...
    cout << "\nLoading z_score statistics file " <<  file_score << " success ! "<<endl; 
...
bool ReadFile_corr(const string &file_cov, const size_t &ns_test, const vector <SNPINFO> &snpInfo, vector< vector<double> >  &LD_ref, map<string, size_t> &mapLDKey2Pos)
{
    ...
    string line;
    char *pch, *nch, *mch;
    string chr, minor, major ,number, rs, mykey ,key , ID;
    long int b_pos;
    double r;
