...
void PARAM::ReadSS (){
	if( (! file_score.empty()) && (!file_cov.empty()) ){
    		...
    		if(ReadFile_score(file_score, snpInfo, mapScoreKey2Pos, mapLDKey2Pos, pval_vec, pos_ChisqTest, U_STAT, SQRT_V_STAT, Z_SCORE,xtx_vec, snp_var_vec, ns_test, ns_total, mbeta, mbeta_SE, indicator_snp, ni_test, maf_level, hwe_level, pheno_var, LD_ref, use_xtx_LD) == false)
    			{ error = true; }
...
void PARAM::CopyPheno (gsl_vector *y) 
{
    ...
    gsl_vector_memcpy(tempc,y);
    gsl_vector_mul(tempc,y);
            //for loop
    double mystd;
    mystd=0.0;
        //doesn't work
        ...
        mystd = sqrt(mystd);
        gsl_vector_scale(y,1/mystd);
        //meeting:scale
        //gsl_matrix_set_col(X, ctest_snp, y;
            //cout some snps
        cout << "success standardize y vector";
	return;
}
