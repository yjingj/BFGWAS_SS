...
void CALCSS::GetSS(gsl_matrix *X, gsl_vector *y, vector< vector<double> > &LD, vector<double> &beta, vector<double> &beta_SE, vector<double> &U_STAT, vector<double> &SQRT_V_STAT, vector<double> &Z_SCORE,vector<double> &pval, vector<pair<size_t, double> > &pos_ChisqTest, vector<double> &xtx_vec, vector<double> &snp_var_vec, vector<double> &ni_effect_vec){
    ...
    double xtx_ij, xty, xtx_i, beta_i, v2, chisq_i, beta_SE_i, r2, z;
    ...
    {
        cout << "\nThe pheno variance = 0, do not calculate and write z_score. \n";
        SQRT_V_STAT.push_back( pheno_var );
        for (size_t i=0; i<ns_test; ++i) {
        //calculate xtx_i
        //Lei's change
        //getGTgslVec(X, xvec_i, snp_pos[i].pos, ni_test, ns_test, SNPmean, CompBuffSizeVec, UnCompBufferSize, Compress_Flag, UcharTable);
        ...
        if(i < (ns_test-1) ){
            ...
            for(size_t j=(i+1); j < ns_test; ++j){
                if( (snp_pos[j].chr == snp_pos[i].chr) && (snp_pos[j].bp <= snp_pos[i].bp + LDwindow) )
                {
                    ...
                    //Lei's change
                    gsl_matrix_get_col(xvec_j, X, snp_pos[j].pos);
...
void CALCSS::WriteSS(const vector< vector<double> > &LD, const vector<double> &beta, const vector<double> &beta_SE, const vector<double> &U_STAT, const vector<double> &SQRT_V_STAT, const vector<double> &Z_SCORE,const vector<double> &pval)
{
    ...
    if(isnan(SQRT_V_STAT[0])==0){
        cout << "\nThe pheno variance != 0, writing z_score. \n";
        if(zipSS){
            cov_file_str +=".LDcorr.txt.gz";
            cov_out = ifopen(cov_file_str, "w", InputFile::BGZF);

            score_file_str += ".z_score.txt.gz";
    ...
    ifprintf(score_out, "#CHROM\tPOS\tID\tREF\tALT\tN\tMAF\tZ_SCORE\tEFFSIZE_BETA\tBETA_SE\tPVALUE\n");  
    ...
    for(size_t i=0; i<ns_test; i++){
        ...
        ifprintf(score_out, "%s\t%ld\t%s\t%s\t%s\t%u\t%g\t%g\t%g\t%g\t%g\n", snp_pos[i].chr.c_str(), snp_pos[i].bp, snp_pos[i].rs.c_str(), snp_pos[i].a_major.c_str(), snp_pos[i].a_minor.c_str(), ni_test, snp_pos[i].maf, Z_SCORE[i], beta[i], beta_SE[i], pval[i]);
        ifprintf(cov_out, "%u\t%s\t%ld\t%s\t%s\t%s\t%u\t%g\t", i,snp_pos[i].chr.c_str(), snp_pos[i].bp, snp_pos[i].rs.c_str(), snp_pos[i].a_major.c_str(), snp_pos[i].a_minor.c_str(), ni_test, snp_pos[i].maf);
    ...
    if(zipSS){
        ...
        printf("Tabixing .z_score.txt.gz files ... \n");
    ...
    if(isnan(SQRT_V_STAT[0])==1 || SQRT_V_STAT[0] == 0)
    {
        cout << "\nThe pheno variance = 0, do not write z_score. \n";
        if(zipSS){
            cov_file_str +=".LDcorr.txt.gz";
            cov_out = ifopen(cov_file_str, "w", InputFile::BGZF);
    ...
    // assuming variants have unique CHR:POS 
    // Lei's note: use Randy's format CHR:POS:REF:ALT
    ifprintf(cov_out, "#ORDER\tCHROM\tPOS\tID\tREF\tALT\tN\tMAF\tCORR\n");
                      //order
    cout << "\nSaving... \n";
    //Write files by the order of chr/bp
    for(size_t i=0; i<ns_test; i++){
        ...
        //lei's note: randy: CHR:POS:REF:ALT
